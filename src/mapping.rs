...
fn uuid_from_vec(bytes: Vec<u8>) -> Result<UUID, UStatus> {
    if bytes.len() < 16 {
    ...
    let msb: [u8; 8] = bytes[..8].try_into().unwrap();
    let lsb: [u8; 8] = bytes[8..].try_into().unwrap();
    Ok(UUID {
...
///
/// The uProtocol message's time-to-live (TTL) is mapped to the MQTT 5
